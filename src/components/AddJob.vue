...
function submitForm() {
  axios
    .post('http://127.0.0.1:8000/api/companies/', companyForm)
    .then((companyDetails) => {
      jobForm.company = companyDetails.data.url
      console.log(companyDetails)
      axios.post('http://127.0.0.1:8000/api/jobs/', jobForm).then((jobDetails) => {
        console.log(jobDetails)
        $toast.success('Job Added')
      })
    })
    .catch((error) => console.log(error))
    .finally(() => console.log('done posting data'))
}
</script>
